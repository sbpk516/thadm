...
- **Falls back gracefully** — no Obsidian? Saves to `~/.thadm/ai-prompts/` as plain markdown
...
1. Install this task from the thadm task gallery
2. (Optional) Connect Obsidian in Settings > Connections — set your vault path
3. The task runs every hour automatically and captures any AI prompts from the last hour
4. Check your journal at `{vault}/thadm/ai-prompts/YYYY-MM-DD.md`

## How it works

The task uses thadm's accessibility tree and OCR data to read what's on your screen. It:
...
- Only reads screen data already captured by thadm
- Prompts are saved to your local filesystem only
- You control the output location (Obsidian vault or ~/.thadm/)
