...
#[tokio::main]
async fn main() {
    eprintln!("[DEBUG STARTUP] ======= main() entered =======");
    ...
    #[cfg(debug_assertions)]
    {
        ...
        if let Err(e) = builder.export(
            ...
            eprintln!("[DEBUG STARTUP] Skipping TypeScript bindings export (not in source tree): {}", e);
        }
    }

    eprintln!("[DEBUG STARTUP] ======= starting Tauri builder =======");
    ...
    let app = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            eprintln!("[DEBUG STARTUP] deep link registration done");
            ...
            eprintln!("[DEBUG STARTUP] macOS menu & space monitor done");
            ...
            info!("[DEBUG STARTUP] tracing/logging initialized successfully");
            eprintln!("[DEBUG STARTUP] tracing/logging initialized");
            ...
            info!("[DEBUG STARTUP] initializing settings store...");
            let store = store::init_store(&app.handle()).unwrap_or_else(|e| {
                error!("[DEBUG STARTUP] Failed to init settings store, using defaults: {}", e);
                store::SettingsStore::default()
            });
            app.manage(store.clone());
            info!("[DEBUG STARTUP] settings store initialized. dev_mode={}, disable_audio={}, port={}", store.dev_mode, store.disable_audio, store.port);

            // Initialize onboarding store
            info!("[DEBUG STARTUP] initializing onboarding store...");
            let onboarding_store = store::init_onboarding_store(&app.handle()).unwrap_or_else(|e| {
                error!("[DEBUG STARTUP] Failed to init onboarding store, using defaults: {}", e);
                store::OnboardingStore::default()
            });
            app.manage(onboarding_store.clone());
            info!("[DEBUG STARTUP] onboarding store initialized. is_completed={}, current_step={:?}", onboarding_store.is_completed, onboarding_store.current_step);

            // Show onboarding window if not completed
            if !onboarding_store.is_completed {
                info!("[DEBUG STARTUP] onboarding NOT completed, showing Onboarding window");
                match ShowRewindWindow::Onboarding.show(&app.handle()) {
                    Ok(_) => info!("[DEBUG STARTUP] Onboarding window shown successfully"),
                    Err(e) => error!("[DEBUG STARTUP] Failed to show Onboarding window: {}", e),
                }
            } else {
                info!("[DEBUG STARTUP] onboarding completed, showing Main window");
                match ShowRewindWindow::Main.show(&app.handle()) {
                    Ok(_) => info!("[DEBUG STARTUP] Main window shown successfully"),
                    Err(e) => error!("[DEBUG STARTUP] Failed to show Main window: {}", e),
                }
            }
            ...
            if !use_dev_mode  {
                info!("[DEBUG STARTUP] dev_mode=false, will attempt sidecar spawn");
                let sidecar_state_clone = sidecar_state_for_init.clone();
                let app_handle_clone = app_handle.clone();
                tauri::async_runtime::spawn(async move {
                    info!("[DEBUG STARTUP] sidecar spawn task started");
                    let mut state_guard = sidecar_state_clone.lock().await;
                    // Initialize the manager if it doesn't exist
                    if state_guard.is_none() {
                        info!("[DEBUG STARTUP] creating new SidecarManager");
                        *state_guard = Some(SidecarManager::new());
                    }
                    let manager = state_guard.as_mut().unwrap();
                    info!("[DEBUG STARTUP] checking if server is already running...");
                    if server_running.await.unwrap_or(false) {
                        info!("[DEBUG STARTUP] server already running, skipping sidecar spawn");
                        return;
                    }
                    info!("[DEBUG STARTUP] server not running, proceeding with sidecar");

                    // Check permissions before spawning sidecar
                    info!("[DEBUG STARTUP] checking permissions...");
                    let permissions_check = permissions::do_permissions_check(false);
                    let disable_audio = store.disable_audio;
                    info!("[DEBUG STARTUP] permissions: screen_recording={:?}, microphone={:?}, accessibility={:?}",
                        permissions_check.screen_recording, permissions_check.microphone, permissions_check.accessibility);
                    ...
                    if !permissions_check.screen_recording.permitted() {
                        warn!("[DEBUG STARTUP] Screen recording permission NOT granted: {:?}. Sidecar will NOT start.", permissions_check.screen_recording);
                        return;
                    }
                    ...
                    if !disable_audio && !permissions_check.microphone.permitted() {
                        warn!("[DEBUG STARTUP] Microphone permission not granted but audio enabled: {:?}. Sidecar will still start.", permissions_check.microphone);
                    }

                    info!("[DEBUG STARTUP] all required permissions OK, spawning sidecar...");
                    if let Err(e) = manager.spawn(&app_handle_clone, None).await {
                        error!("[DEBUG STARTUP] Failed to spawn initial sidecar: {}", e);
                    } else {
                        info!("[DEBUG STARTUP] sidecar spawned successfully");
                    }
                });
            } else {
            ...
            info!("[DEBUG STARTUP] starting update check service...");
            let update_manager = start_update_check(&app_handle, 5)?;
            info!("[DEBUG STARTUP] update check service started");
            ...
            #[cfg(target_os = "macos")]
            {
                info!("[DEBUG STARTUP] setting activation policy to Regular");
                app.set_activation_policy(tauri::ActivationPolicy::Regular);
            }

            // Initialize global shortcuts
            info!("[DEBUG STARTUP] initializing global shortcuts...");
            let app_handle_clone = app_handle.clone();
            tauri::async_runtime::spawn(async move {
                if let Err(e) = initialize_global_shortcuts(&app_handle_clone).await {
                    warn!("[DEBUG STARTUP] Failed to initialize global shortcuts: {}", e);
                } else {
                    info!("[DEBUG STARTUP] global shortcuts initialized successfully");
                }
            });
            info!("[DEBUG STARTUP] ======= setup() closure completed successfully =======");
            Ok(())
        })
        .build(tauri::generate_context!())
        .expect("error while building tauri application");

    eprintln!("[DEBUG STARTUP] Tauri app built, entering run loop");

    // set_tray_unhealth_icon(app.app_handle().clone());
    app.run(|app_handle, event| match event {
        tauri::RunEvent::Ready { .. } => {
            eprintln!("[DEBUG STARTUP] ======= RunEvent::Ready - app is ready! =======");
