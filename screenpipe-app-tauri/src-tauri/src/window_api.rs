            ...
            if id.label() == RewindWindowId::Main.label() {
                    ...
                    #[cfg(target_os = "macos")]
                    {
                        ...
                        app.run_on_main_thread(move || {
                            ...
                            use tauri_nspanel::cocoa::foundation::{NSArray, NSRect};

                            if let Ok(panel) = app_clone.get_webview_panel(RewindWindowId::Main.label()) {
                                use objc::{msg_send, sel, sel_impl};

                                // Use primary screen (screens[0]) instead of cursor position.
                                // Cursor tracks to the tray icon's monitor which may differ from
                                ...
                                unsafe {
                                    ...
                                    if screen_count > 0 {
                                        let primary_screen: id = NSArray::objectAtIndex(screens, 0);
                                        let frame: NSRect = NSScreen::frame(primary_screen);
                                        info!("Moving panel to primary screen at ({}, {}), size {}x{}",
            ...
            ShowRewindWindow::Main => {
                ...
                let window = {
                    ...
                    let (monitor, position) = {
                        // Use primary monitor instead of cursor position.
                        // Cursor tracks to the tray icon's monitor which may differ
                        // from the monitor the user is actually looking at.
                        {
                            let target_monitor = app.primary_monitor().unwrap().unwrap();
                            let target_position = (0.0_f64, 0.0_f64);
