...
pub async fn spawn_screenpipe(
    ...
    // License gate: if trial expired and no valid license, override config to disable capture.
    let license = LicenseStore::get(&app).ok().flatten().unwrap_or_default();
    let license_blocked = !license.is_recording_allowed();
    if license_blocked {
        info!("[LICENSE] trial expired and no valid license — recording disabled");
        let _ = app.emit("license-recording-blocked", ());
    }

    let mut recording_config = store.to_recording_config(data_dir);
    if license_blocked {
        recording_config.disable_vision = true;
        recording_config.disable_audio = true;
    }
