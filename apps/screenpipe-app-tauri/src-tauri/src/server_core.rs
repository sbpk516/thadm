    ...
    pub async fn start(
        ...
        if config.audio_transcription_engine == AudioTranscriptionEngine::Deepgram {
            ...
            if has_personal_key {
                ...
                // THADM: disabled — screenpipe cloud Deepgram proxy. We never
                // route audio through screenpi.pe. Without a personal key,
                // ensure no proxy URL/token leaks from a prior session.
                std::env::remove_var("DEEPGRAM_API_URL");
                std::env::remove_var("CUSTOM_DEEPGRAM_API_TOKEN");
