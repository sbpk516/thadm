...
- "meeting / call / conversation / what did I/they say" → search with content_type: "audio", no q param (for past meetings/calls captured by thadm)
...
- Frame: \`[10:30 AM — Chrome](thadm://frame/12345)\` — only with a real frame_id from results
- Timeline (audio): \`[meeting at 3pm](thadm://timeline?timestamp=2024-01-15T15:00:00Z)\` — exact timestamp from audio results
  ...
  const exportChatAsMarkdownFile = async () => {
    ...
    try {
      const filePath = await saveDialog({
        ...
        defaultPath: `thadm-chat-${new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19)}.md`,
      });
      if (filePath) {
