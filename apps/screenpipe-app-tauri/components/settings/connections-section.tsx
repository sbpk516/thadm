...
async function buildMcpConfig(opts?: { forceNpx?: boolean }): Promise<McpCommand> {
  if (opts?.forceNpx) return { command: "npx", args: ["-y", "thadm-mcp"] };
  try {
...
async function uninstallClaudeMcp(): Promise<void> {
  ...
  if (!servers?.thadm) return;
  delete servers.thadm;
  await writeFile(configPath, new TextEncoder().encode(JSON.stringify(config, null, 2)));
}
      ...
      <div className="flex gap-2">
        {!isSaved && (
          <Button onClick={handleConnect} disabled={!hasCredentials || status === "connecting"} variant={status === "error" ? "outline" : "default"} size="sm" className="gap-1.5 h-7 text-xs normal-case font-sans tracking-normal">
            {status === "connecting" ? (<><Loader2 className="h-3 w-3 animate-spin" />testing...</>)
             : status === "error" ? (<>retry</>)
  ...
  const renderPanel = () => {
    ...
    switch (selected) {
      case "claude": return <ClaudePanel
        onConnected={() => { localStorage.setItem("thadm_claude_connected", "true"); setClaudeInstalled(true); }}
        onDisconnected={() => { localStorage.removeItem("thadm_claude_connected"); setClaudeInstalled(false); }}
      />;
      ...
      default:
        ...
        return (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground">
              couldn&apos;t load connection metadata. the local thadm server may be starting up.
            </p>
            <Button
              size="sm"
              variant="outline"
