...
```bash
# step 1: sync data
bunx @screenpipe/sync --daemon --remote user@your-server:~/.thadm/
...
query the user's screen history via SQLite at ~/.thadm/db.sqlite.
...
- check data exists: `sqlite3 ~/.thadm/db.sqlite "SELECT COUNT(*) FROM ocr_text;"`
